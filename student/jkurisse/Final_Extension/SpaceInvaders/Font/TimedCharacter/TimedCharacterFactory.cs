...
namespace SE456
{
    class TimedCharacterFactory
    {
        ...
        static public void Install(SpriteGame.Name name, SpriteBatch pSB_AlienSelect, float deltaTimeToTrigger, float delayTime, float xPos, float yPos, float red, float green, float blue)
        {
            TimedCharacterFactory pInstance = TimedCharacterFactory.privInstance();
            // future use

            //TimedSpriteCmd pCmd_old = null;

            TimedSpriteCmd pCmd = new TimedSpriteCmd(name, pSB_AlienSelect, xPos, yPos, red, green, blue);

            float time = deltaTimeToTrigger + delayTime;
            TimerEventMan.Add(TimerEvent.Name.TimedCharacter, pCmd, time);
