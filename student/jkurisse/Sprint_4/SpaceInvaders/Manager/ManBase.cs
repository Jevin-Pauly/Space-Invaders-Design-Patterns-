...
namespace SE456
{
    ...
    abstract public class ManBase
    {
        ...
        public NodeBase basePartialAdd(float priority)
        {
            Iterator pIt = poReserve.GetIterator();
            Debug.Assert(pIt != null);
            ...
            // copy to active
            poActive.AddPriority(pNodeBase, priority);

            // YES - here's your new one (may its reused from reserved)
            return pNodeBase;
        }

        public void basePartialRemove(NodeBase pNode)
        {
            Debug.Assert(pNode != null);
        ...
        abstract protected bool derivedCompare(NodeBase pLinkA, NodeBase pLinkB);
        abstract protected void derivedWash(NodeBase pLink);
        abstract protected void derivedDumpNode(NodeBase pLink);
